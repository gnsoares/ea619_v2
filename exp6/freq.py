...
# freqeuncia da senoide de entrada
omega=25;
...
for time_i in timeSim:
    ...
    sim.simxCallScriptFunction(clientID,
                                ...
                                'addForceTo',
                                [h],
                                [0, 0, 0, 0, 0, -1*np.sin(omega*time_i)],
                                [],
                                bytearray(),
    ...
    position = np.append(position, posTopPlate[2])
...
df.to_csv('sim_200g_'+str(omega)+'rads.csv', index=False)
